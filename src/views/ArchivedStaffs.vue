<template>
  <div class="p-20 !pt-14 space-y-10">
        ...
        <table class="w-full rounded-md overflow-hidden" id="staffsTable">
          ...
          <tbody v-if="filteredStaff()?.length">
            ...
            >
              ...
              <td class="border-x text-center py-2 capitalize">
                {{ moment(staff.birthDate).format("ll") }}
              </td>
              <td class="border-x text-center py-2 capitalize">
                {{ moment(staff.dateStarted).format("ll") }}
              </td>
              <td class="border-x text-center py-2 capitalize">
                {{ staff.address }}
              </td>
              ...
              <td class="border-x text-center py-2">
                <div class="flex items-center gap-x-2 justify-center text-xl">
                  ...
                  <Icon
                    ...
                    @click="restoreStaff(staff.id, index)"
                  />
                </div>
              </td>
            </tr>
          ...
          <tbody v-else>
            <tr>
              <td class="border text-center py-2" colspan="9">
                No staffs to show
              </td>
            </tr>
          </tbody>
    ...
    <EditStaff
      v-if="willEditStaff"
      @closeModal="willEditStaff = false"
      :staffDetailsToEdit="staffToEditDetails"
    />
    <DeleteModal
      v-if="willDeleteStaff"
      :type="'Staff'"
      @closeModal="willDeleteStaff = false"
