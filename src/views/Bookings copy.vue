    ...
    <div class="bg-white w-full h-fit p-5 rounded-lg shadow space-y-5">
      ...
      <div class="w-full overflow-x-auto">
        <table class="w-[170%] rounded-md overflow-hidden" id="bookingsTable">
          ...
          <tfoot class="hidden print:block">
            <tr>
              <td colspan="2" class="text-start font-bold py-2">Total:</td>
              <td :colspan="selectedColumns.length - 2" class="text-start font-bold py-2">
                {{ formatCurrency(filteredBookings().reduce((sum, booking) => sum + (booking.totalPrice || 0), 0)) }}
              </td>
              <td></td>
            </tr>
          </tfoot>
