<template>
  <div class="p-20 !pt-14 space-y-10">
    ...
    <div class="flex">
      <button
        class="ml-auto border border-custom-primary text-custom-primary px-3 py-1 rounded hover:shadow-md"
        @click="willAddItem = true"
      >
        Add Item
      </button>
    </div>
    <div class="bg-white w-full h-fit p-5 rounded-lg shadow space-y-5">
      <div class="flex justify-start gap-x-2">
        ...
        <button
          class="bg-green-500 text-white px-3 rounded ml-auto"
          @click="generatePDF"
        >
          Generate PDF
        </button>
          ...
          <tbody v-if="filteredItems()?.length">
            ...
            >
              ...
              <td class="border-x text-center py-2 capitalize">
                <button
                  class="bg-green-500 px-3 rounded text-white"
                  :class="{ 'bg-red-500': item.quantity < 50 }"
                >
    ...
    <EditItem
      ...
      :itemDetailsToEdit="itemToEditDetails"
    />
    <DeleteModal
      v-if="willDeleteItem"
      :type="'Item'"
      @closeModal="willDeleteItem = false"
...
const getItems = async () => {
  try {
    const q = query(itemsRef, where("type", "==", "reusable"));
...
const saveNotifiedItems = () => {
  localStorage.setItem(
    "notifiedItems",
    JSON.stringify(Array.from(notifiedItems.value))
  );
};
...
const generatePDF = () => {
  ...
  doc.save("reusable-inventory.pdf");
};
</script>
